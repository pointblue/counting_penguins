...
wd <-
  "z:/informatics/s031/analyses/counting_penguins/scripts/"
...
#############################################################################################
#If you want, you can get data from .json files that SageMaker creates in closer to real time 
#the manifest file is updated only once per hour or so?
#this is more accurate because it updates as the workers create labels:
prefix2 <- "PenguinCounting/croz_20211127/label_sample/adpe-label-test3/annotations/worker-response/iteration-1/"
...
file_filter <- filter(files, str_sub(files$Key,-1,-1)!="/")

print(paste(nrow(file_filter),"tiles labeled so far."))
prop_labeled <-(nrow(file_filter)/nrow(df))*100
print(paste(format(round(prop_labeled, 2), nsmall=2), "% complete."))

###################################################
#summarize more about the job from the .json files
#This part needs work!
###################################################
n_tiles <- length(file_filter$Key)
l1 = 0
l2 = 0 
l3 = 0
...
for(tile in 1:n_tiles) {
  ...
  dfj <- rawToChar(get_object(
    file_filter$Key[tile],
    bucket
    ))
  #print(paste("Tile: ",file_filter$Key[tile]))
  
  #dfj<-suppressWarnings(read.table("json_test.json"))
  
  jstr1 <- fromJSON(as.character(dfj[1])) #There should only be one object in each of these

  n_labels<-length(jstr1$answers[[1]]$answerContent$boundingBox$boundingBoxes)  

  for (i in 1:n_labels) {
    label<-jstr1$answers[[1]]$answerContent$boundingBox$boundingBoxes[[i]]$label
    #print(label)
    
    if (label == "ADPE_a") {l1=l1+1
      } else if (label == "ADPE_a_stand") {
      l2=l2+1
      } else if (label == "no_ADPE") {
      l3=l3+1
      }
    }
  worker<-jstr1$answers[[1]]$workerId
  print(paste("Tile: ",tile, "Worker:",worker))
}



print(paste0("Total tiles: ",n_tiles))
print(paste0("ADPE_a: ", l1))
print(paste0("ADPE_a_stand: ", l2))
print(paste0("no_ADPE: ", l3))


######################################
#same as above but as a function call
#####################################
get_tile_info(ff = file_filter)

get_tile_info <- function(ff) {
  
    n_tiles <- length(ff$Key)
    l1 = 0
    l2 = 0 
    l3 = 0
...
print(paste0("Total tiles: ",tile_info[1]))
print(paste0("ADPE_a: ", tile_info[2]))
print(paste0("ADPE_a_stand: ", tile_info[3]))
print(paste0("no_ADPE: ", tile_info[4]))
...
#can change the file you want to read from using:
#yolo_tiles_summary<- read.csv("../data/labels/croz_20201129/yolo_tiles_summary.csv")
